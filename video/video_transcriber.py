...
def transcribe_video(video_file_path: str, max_duration_sec: int = 3600) -> TranscriptionResults:
    ...
    for local_mp3_file in local_mp3_files:
        gcs_file = gcs_upload_file.submit(local_file_path=local_mp3_file, gcs_bucket=gcs_bucket, gcs_folder=gcs_folder)
...
def transcribe_chirp(
    ...
    bucket = variables.get("gcs_bucket")
    folder = variables.get("gcs_folder")
    ...
    request = cloud_speech.BatchRecognizeRequest(
        ...
        recognition_output_config=cloud_speech.RecognitionOutputConfig(
            gcs_output_config=cloud_speech.GcsOutputConfig(
                uri=f'gs://{bucket}/{folder}/transcriptions/'),
        ),
